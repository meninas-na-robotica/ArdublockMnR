...
window.addEventListener('load', function load(event) {
	...
	document.getElementById('btn_connect').onclick = function(event) {
		...
		if (connexion){
			document.getElementById('btn_connect').innerHTML="<span class='fa fa-play'></span>"
			document.getElementById('btn_envoi').disabled=true
			s_p.close(function (err) { moniteur.innerHTML += '---<br>' })
			connexion = false
		} else {
			s_p = new SerialPort(com,{baudRate:baud, autoOpen:false})
			document.getElementById('btn_connect').innerHTML="<span class='fa fa-pause'></span>"
			document.getElementById('btn_envoi').disabled=false
			s_p.open(function (err) { if(!err) moniteur.innerHTML += '---<br>'; else moniteur.innerHTML += '--- ERROR OPENING SERIAL PORT: '+err.message+' ---<br>' })
			connexion = true
