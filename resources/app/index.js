...
window.addEventListener('load', function load(event) {
	...
	document.getElementById('versionapp').textContent = " ESP Block" + appVersion
	function uploadOK(){
	...
	$('#btn_forum').on('click', function(){
		shell.openExternal('http://ardublock.ru/')
	})
		...
		if (prog == "python") {
			...
			exec('python -m pyflakes ./py/sketch.py', {cwd:"./compilation/python"}, function(err, stdout, stderr){
				if (stderr) {
					...
					if (rech.test(stderr)){
						messageDiv.style.color = '#ff0000'
						messageDiv.innerHTML = Blockly.Msg.error
	//	$('#message').modal('show');
		setTimeout(function() {
		$('#message').modal('hide');
		}, 2000);
	...
	//	$('#message').modal('show');
		...
		$('#message').modal('hide');
			    ...
			    messageDiv.style.color = '#009000'
				messageDiv.innerHTML = Blockly.Msg.check + ':  Code is ready to upload'
			 //	$('#message').modal('show');
				setTimeout(function() {
    			$('#message').modal('hide');
				}, 3000);
	...
	$('#btn_flash').on('click', function(){
		...
		if ( com == "com" ){
			messageDiv.style.color = '#ff0000'
			messageDiv.innerHTML = Blockly.Msg.com2 // укажите порт платы
				//$('#message').modal('show');
				setTimeout(function() {
    			$('#message').modal('hide');
				}, 3000);
		...
		if ( localStorage.getItem('verif') == "false" ){
			...
			if (error) {
						...
						messageDiv.style.color = '#ff0000'
						//messageDiv.innerHTML = error.toString()	
				messageDiv.innerHTML = Blockly.Msg.comerror // Ошибка компиляции! Проверьте программу! 
				//$('#message').modal('show');
				setTimeout(function() {
    			$('#message').modal('hide');
				}, 3000);
			...
			//exec('flash.bat ' + cpu + ' ' + prog + ' '+ com + ' ' + speed, {cwd: './compilation/arduino'} , function(err, stdout, stderr){
				if (err) {
					messageDiv.style.color = '#ff0000'
				//	messageDiv.innerHTML = err.toString()
				messageDiv.innerHTML = Blockly.Msg.uploadcom // ошибка ком порта загрузки 
		//$('#message').modal('show');
	 	setTimeout(function() {
	 	$('#message').modal('hide');
	 	}, 3000);
		...
		if ( prog == "python" ) {
			if ( cpu == "cortexM0" ) {
				...
				if (disk!="") {
					...
					messageDiv.style.color = '#000000'
					messageDiv.innerHTML = 'Connect micro:bit!'
				}
			} else {
	...
	ipcRenderer.on('saved-bin', function(event, path){
		if (path === null) {
			...
			var code = Blockly.Xml.domToPrettyText(xml)
			var res = path.split(".")
			fs.writeFile(res[0]+'.txt', code, function(err){
				if (err) return console.log(err)
			})
