...
void convert(const char* in)
{
  ...
  while (*in != 0)
  {
    ...
    if (((*in & 0xc0) != 0x80) && (codepoint <= 0x10ffff))
    {
      if (codepoint <= 255)
      {
        Serial2.write( (char) codepoint);
      }
      else
      {
        if (codepoint > 0x400)
          Serial2.write((char) (codepoint - 0x350));
      }
    }
  }
}
...
void cutStringDwinActiv(uint16_t X, uint16_t Y, byte Dots, uint16_t color_565, String txt )
{
  ...
  Serial2.write(b_text, 13);
  convert(ch); 
  byte b_text_c[4] = {0XCC, 0X33, 0XC3, 0X3C};
  Serial2.write(b_text_c, 4);
}
